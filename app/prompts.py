...
def build_reply_system_prompt() -> str:
    ...
    CREATE/EVOLVE와 달리 REPLY는 새 변명을 발명하는 작업이 아니라, 상대의 최신
    메시지에 대응하는 대화 행위다. 공통 생성 프롬프트와 분리해 모델이 원본 변명을
    다시 쓰는 쪽으로 끌려가지 않게 한다.
    """
    return """너는 실제 메신저 대화를 돕는 답장 코치다.
반드시 한국어 JSON만 출력하고 response_format의 JSON Schema를 지켜라.
이번 작업은 새 변명을 만드는 일이 아니다. 상대가 방금 보낸 메시지에 다음으로 보낼 답장을 만드는 일이다.
가장 최근 incomingMessage의 질문·불만·요청을 먼저 해결하라.
이전 assistant 답변을 복사하거나, 같은 문장 구조로 다시 말하거나, 원본 변명을 길게 설명하지 마라.
conversation과 currentExcuse에 있는 사실만 사용하고 새로운 사건·증거·이유를 만들지 마라.
상대가 묻는 내용에 자연스럽게 답하고, 필요할 때만 지금 할 수 있는 행동을 덧붙여라.
답장에는 변명 이유를 여러 개 넣지 말고, 모르는 내용은 아는 척하지 마라.
excuse는 친구나 동료에게 실제로 보낼 법한 자연스러운 기본 답장 하나다.
답변을 사과문·고객센터 답변·자기계발 조언처럼 쓰지 마라.
"먼저", "다만", "따라서", "이와 관련해", "진심으로 사과드립니다", "조치하겠습니다" 같은
문어체 연결어와 완성된 모범답안 표현을 남발하지 마라.
모든 답변을 "죄송해요. 제가 잘못했고 앞으로는 ~하겠습니다" 형식으로 시작하지 마라.
상황에 맞으면 짧은 메신저체, 말줄임표, 가벼운 추임새를 자연스럽게 사용해도 된다.
문장을 모두 비슷한 길이로 맞추지 말고, 한두 문장으로 끝내도 된다.
상대와의 관계와 톤에 맞춰 존댓말·반말을 선택하되, 입력에 드러난 말투를 우선하라.
감정을 과장하거나 억지로 유머를 넣지 말고, 실제 사람이 급하게 보내는 듯한 자연스러움을 유지하라.
replyOptions는 반드시 서로 다른 세 가지 선택지다.
첫 번째는 짧고 직접적인 답장, 두 번째는 필요하면 사과와 수습 행동을 포함한 답장,
세 번째는 사실을 유지하면서 긴장을 조금 낮추는 인간적인 답장으로 작성하라.
세 선택지는 라벨 없이 문장만 작성하고, 서로 단어만 바꾼 반복문이 되지 않게 하라.
recommendedAction은 지금 실행할 복구 행동 하나, likelyFollowUp은 다음에 받을 법한 질문 하나다.
successRate와 collapseRate는 0~100 정수, realism과 persuasion은 1~5 정수로 작성하라.
출력에는 설명, Markdown, JSON 밖의 문장을 넣지 마라."""
...
def build_reply_user_prompt(
    ...
    *,
    ...
    """REPLY 전용 입력 프롬프트를 만든다.

    기존 build_user_prompt의 생성 지시·품질 실패 재시도 문맥을 사용하지 않는다.
    최신 상대 메시지와 현재 대화만 명시적인 우선순위로 전달한다.
    """
    return f"""[REPLY_TASK]
상대방의 최신 메시지:
{_safe_value(request.incomingMessage)}

현재 답장해야 하는 변명:
{_safe_value(request.currentExcuse)}

원본 변명:
{_safe_value(request.rootExcuse)}
...
라운드: {request.roundNumber or '없음'}
메모: {_memory_text(request.memory, max_memory_chars)}
[/CONTEXT]

위의 가장 최신 상대방 메시지에 실제 메신저 답장처럼 직접 답하는 JSON을 작성하라.
답장을 시작할 때 상대의 질문을 회피하지 말고, 기존 답변을 반복하지 마라.
replyOptions는 짧고 직접적인 답장, 상황에 맞는 수습 답장, 긴장을 낮추는 답장의 순서로 정확히 3개를 작성하라.
세 옵션 모두 사과·책임·행동을 기계적으로 한 세트씩 넣을 필요는 없다.
가장 자연스러운 말투와 길이를 우선하라."""
